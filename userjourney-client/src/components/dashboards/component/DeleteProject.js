...
export default function DeleteProject({ openRemove, setOpenRemove, project, onState,setOnState}) {
   

    const handleOnClick = (data) => { 

        axios
            .delete(`http://localhost:5000/api/v1/projects/delete/${project?.id}`)
            .then((res) => {
                ...
                <Fade in={openRemove}>
                    <Box sx={style} className="modal-main">
                        ...
                        <Box>
                            ...
                            Are you sure you want to remove <span className="remove_project_name">{project?.project_name}</span>?This action cannot be undone
                            </Typography>
                        </Box>
