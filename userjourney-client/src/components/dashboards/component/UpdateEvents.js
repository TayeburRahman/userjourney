...
export default function UpdateEvents({ event, open, setOpen, setEventStatus, eventStatus }) {
  ...
  const onSubmit = (data) => {
    ...
    if (avatar) {
      axios
        .put(`http://localhost:5000/api/v1/event/update/avatar/${event?._id}`, { user, formData, avatar },
          {
            headers: {
      ...
      axios
        .put(`http://localhost:5000/api/v1/event/update/${event?._id}`, { user, formData })
        .then((res) => {
          alert("Successfully Update Event!");
          setOpen(false);
          setEventStatus(eventStatus === 1 ? 0 : 1)
          ...
          <Box sx={style}>
            ...
            <Box>
              <form onSubmit={handleSubmit(onSubmit)} className="d-grid">
                ...
                <input className="event-input" defaultValue={event?.title} {...register("title")} />

                <label className="mt-2">Spaces Available: </label>
                <input
                  type="number"
                  className="event-input"
                  defaultValue={event?.spacesAvailable}
                ...
                <textarea defaultValue={event?.description} {...register("description", { required: true })} />
