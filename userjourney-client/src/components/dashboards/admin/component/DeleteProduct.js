...
export default function DeleteProducts({ openRemove, setOpenRemove, removeData, onState, setOnState }) {
    const handleOnClick = (data) => {
        axios
            .delete(`http://localhost:5000/api/v1/product/delete_product/${removeData?.id}`)
            .then((res) => {
                        ...
                        <Box>
                            <Box>
                                <Typography className="remove-sm-text">
                                    Are you sure you want to remove <span className="remove_project_name">{removeData?.name}</span>?This action cannot be undone
                                </Typography>
                            </Box>
